...
def process_batch(
    ...
    # 批量检测人脸
    detected_faces_batch = sfd_detector.detect_from_batch(
    ...
    for idx, (frame, faces) in enumerate(zip(frames, detected_faces_batch)):
        if len(faces) > 0:
    ...
    if face_crops:
        face_tensors = torch.stack([
            torch.Tensor(crop).permute(2, 0, 1) / 255.0 
        ...
        for i in range(0, len(face_tensors), batch_size):
            batch = face_tensors[i:i + batch_size]
            with torch.no_grad():
                pred = emonet(batch)
                predictions.append({
        ...
        # 合并预测结果
        expressions = torch.cat([p['expression'] for p in predictions])
        valences = torch.cat([p['valence'] for p in predictions])
        arousals = torch.cat([p['arousal'] for p in predictions])
        
        emotion_probs = nn.functional.softmax(expressions, dim=1)
        predicted_emotions = [
            emotion_classes[e.item()] 
            for e in torch.argmax(emotion_probs, dim=1)
        ]
    ...
    for i, timestamp in enumerate(timestamps):
        if i in valid_indices:
            ...
            results.append({
                ...
                '帧号': frame_indices[i],
                '情绪': predicted_emotions[idx],
                '效价': valences[idx].item(),
                '唤醒度': arousals[idx].item(),
                '是否检测到人脸': True
            })
        else:
            results.append({
...
def main():
    ...
    try:
        ...
        for i in range(0, len(list_frames_rgb), batch_size):
            batch_frames = list_frames_rgb[i:i + batch_size]
            batch_timestamps = timestamps[i:i + batch_size]
            batch_indices = list(range(i, min(i + batch_size, len(list_frames_rgb))))
